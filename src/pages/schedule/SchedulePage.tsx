...
export default function SchedulePage({ onSelectActivity }: Props) {
  ...
  return <div className="schedule-page flex min-h-0 flex-1 flex-col overflow-hidden">
    ...
    {loading && <div className="p-12 text-center text-[13px]" style={{ color: 'var(--c-muted)' }}>Loading connected schedule…</div>}
    ...
    {!loading && !error && workspace && !workspace.activities.length && <div className="p-12 text-center"><h2 className="text-[16px] font-bold" style={{ color: 'var(--c-text)' }}>No schedule activities</h2><p className="mt-1 text-[13px]" style={{ color: 'var(--c-muted)' }}>This project has no active schedule activities.</p></div>}
