...
import { useRole, ROLE_LABELS, type UserRole } from '../context/RoleContext'
    ...
    >
      ...
      <div className="px-4 py-3.5" style={{ borderBottom: '1px solid var(--c-border)' }}>
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="text-[13px] font-semibold" style={{ color: 'var(--c-text)' }}>{user.name}</div>
            <div className="text-[11px]" style={{ color: 'var(--c-muted)' }}>{user.roleLabel}</div>
            <div className="text-[11px]" style={{ color: 'var(--c-subtle)' }}>{user.project}</div>
          </div>
        </div>
      </div>
    ...
    <div style={{ overflow: 'auto', flex: 1, minHeight: 0 }}>
      <div style={{ maxWidth: 640, margin: '0 auto', padding: '24px 28px 48px' }}>
        ...
        <div className="mb-5 flex items-center gap-4 rounded-[16px] p-5" style={{ background: 'var(--c-card)', border: '1px solid var(--c-border)' }}>
          ...
          <div>
            <div className="text-[18px] font-bold" style={{ color: 'var(--c-text)' }}>{user.name}</div>
            <div className="text-[13px]" style={{ color: 'var(--c-muted)' }}>{user.roleLabel} · {user.project}</div>
            <div className="text-[12px]" style={{ color: 'var(--c-subtle)' }}>{user.email}</div>
          </div>
        </div>
        ...
        {[
          ...
          { label: 'Role', value: user.roleLabel, note: 'Contact your administrator to change your role.' },
          { label: 'Project', value: user.project },
          { label: 'Assigned Disciplines', value: user.disciplines },
        ].map((f) => (
        ...
        <div className="mb-4 rounded-[12px] px-4 py-3" style={{ background: 'var(--c-card)', border: '1px solid var(--c-border)' }}>
          <div className="mb-2 text-[11px] font-bold uppercase" style={{ color: 'var(--c-subtle)', letterSpacing: '0.08em' }}>Theme Preference</div>
        ...
        <div className="rounded-[12px] px-4 py-3" style={{ background: 'var(--c-card)', border: '1px solid var(--c-border)' }}>
          <div className="mb-3 text-[11px] font-bold uppercase" style={{ color: 'var(--c-subtle)', letterSpacing: '0.08em' }}>Notification Preferences</div>
          {['Review required', 'Conflict detected', 'Report completed', 'Clarification received', 'Verification confirmed'].map((pref) => (
            <div key={pref} className="flex items-center justify-between py-2" style={{ borderBottom: '1px solid var(--c-border)' }}>
              <span className="text-[13px]" style={{ color: 'var(--c-text)' }}>{pref}</span>
              <div
                className="flex h-5 w-9 cursor-pointer items-center rounded-full transition-colors"
                style={{ background: '#F46F29', padding: '0 2px' }}
                role="switch"
                aria-checked="true"
                aria-label={pref}
              >
                <div className="ml-auto h-3.5 w-3.5 rounded-full bg-white shadow-sm" />
              </div>
            </div>
          ))}
        </div>
